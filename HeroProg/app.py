...
def editar_heroi(id_heroi):
    ...
    if request.method == 'POST':
        nome_heroi = request.form['nome_heroi']
        ...
        classe_ranking = request.form['classe_ranking']
        posicao_ranking = request.form['posicao_ranking'] 

        cursor.execute(
            "UPDATE herois " \
            "SET nome_heroi=%s, id_classe=%s, imagem_url=%s, habilidades=%s, forca=%s, defesa=%s, velocidade=%s, id_time=%s, classe_ranking=%s, posicao_ranking=%s " \
            "WHERE id_heroi = %s", 
            (nome_heroi, id_classe, imagem_url, habilidades, forca, defesa, velocidade, id_time, classe_ranking, posicao_ranking, id_heroi)
        )
        conn.commit()
        cursor.close()
        conn.close()
        return redirect(url_for('ver_heroi'))
    ...
    return render_template('editar_heroi.html', heroi=heroi)
